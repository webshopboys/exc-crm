...
public class DocumentumView extends Commontroller implements Serializable {
	...
	public StreamedContent getDocumentStream(final String documentKey) throws Exception {
		try {
			...
			// System.out.println("getDocumentStream " + documentKey + " " +
			// docType + " " + docid);
			...
			if (docType.equals(DocBean.KEY_DR)) {
				...
				if (doc != null)
					...
					generator.setContent(doc.getDocumentBin())
							.setMimeType(
									AttachmentGenerator.calulateMimeType(ObjectUtils.getFileName(doc.getDocumentUrl())))
			...
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Hiba",
					"A letöltés nem sikerült: " + e.getLocalizedMessage());
	...
	public String getTemplateFields() {
		return hu.exclusive.crm.report.ReportParameterProvider.DOCFIELDS_NAMES.getInfo();
	}
}