...
public class StaffController extends Commontroller implements Serializable {
	...
	public void init() {
		...
		this.staffModel = new LazyDataModel<Staff>() {
			...
			@Override
			public List<Staff> load(int first, int pageSize, List<SortMeta> multiSortMeta,
					Map<String, Object> filters) {
	...
	private List<Staff> pageDataTable(LazyDataModel<Staff> dataModel, int first, int pageSize, String sortField,
		...
		// System.err.println("pageDataTable filter: " +
	...
	public void saveStaff() {
		...
		try {
			...
			// {
			// error("Hiányos adatok", "A teljes név és login név is kötelező,
			// és nem lehet már létező!", "userForm");
			// return;
			// }
	...
	public void closeActiveStaff() {
		// FIXME az adatlap bezárása és a lista újratltése a korábbi
		// felterekkel. Vagyis mi van, ha új munkatárs? Akkor is.
		...
		try {
			...
			if (contract != null) {
				...
				generator.setContent(content)
						.setAttachmentName(ObjectUtils.cleanFilename(activeStaff.getFullName() + " "
								+ contract.getDocumentType() + " " + ObjectUtils.getDateTime(new Date()) + ".docx"))
			...
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Hiba",
					"A kitöltés nem sikerült: " + e.getLocalizedMessage());
